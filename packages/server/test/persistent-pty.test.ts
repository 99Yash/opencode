    ...
    Effect.gen(function* () {
      ...
      const first = Schema.decodeUnknownSync(PersistentPty.Info)(
        (yield* request(base, "POST", `/api/experimental/session/${sessionID}/terminal`, {
          command: "/usr/bin/env",
          args: ["/bin/sh", "-c", "stty -echo; printf 'terminal-one:%s\\n' \"$PWD\"; cat"],
          cwd: process.cwd(),
          title: "first",
          env: {},
        })).data,
      ...
      expect(yield* waitForText(base, first.id, "terminal-one")).toContain(`terminal-one:${process.cwd()}`)
...
for (const shell of ["/bin/sh", "/missing/server/zsh"]) {
  smoke(
    `defaults persistent terminals to the session directory and server shell (${shell})`,
    () =>
      Effect.gen(function* () {
        const fixture = yield* testDirectory("xdg", shell)
        const server = yield* ServerProcess.start<never, never>({
          hostname: "127.0.0.1",
          port: 0,
          password: "secret",
        ...
        const base = HttpServer.formatAddress(server.address)
        const sessionID = Session.ID.make("ses_persistent_pty_defaults")
        const input = {
          args: ["-c", "printf 'cwd:%s\\n' \"$PWD\"; cat"],
          title: "default shell",
          env: { SHELL: "/missing/client/zsh" },
        }
        const missing = yield* request(base, "POST", `/api/experimental/session/${sessionID}/terminal`, input).pipe(
          Effect.flip,
        )
        ...
        const terminal = Schema.decodeUnknownSync(PersistentPty.Info)(
          (yield* request(base, "POST", `/api/experimental/session/${sessionID}/terminal`, input)).data,
        )
        expect(terminal).toMatchObject({ cwd: fixture.root, args: input.args, status: "running" })
        expect(terminal.cwd).not.toBe(process.cwd())
        if (shell === "/bin/sh") expect(terminal.command).toBe("/bin/sh")
        expect(existsSync(terminal.command)).toBeTrue()
        expect(terminal.command).not.toBe("/missing/server/zsh")
        expect(terminal.command).not.toBe(input.env.SHELL)
        expect(yield* waitForText(base, terminal.id, `cwd:${fixture.root}`)).toContain(`cwd:${fixture.root}`)
      }),
    20_000,
  )
}

function testDirectory(mode: "xdg" | "override", shell?: string) {
