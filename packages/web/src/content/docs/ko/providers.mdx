...
:::note
"I'm sorry, but I cannot assist with that request" 같은 오류가 발생하면 Azure 리소스에서 콘텐츠 필터를 **DefaultV2**에서 **Default**로 변경해 보세요.
:::

1. [Azure 포털](https://portal.azure.com/)로 이동하여 **Azure OpenAI** 리소스를 만드세요. **리소스 이름**이 필요하며, 이는 API 엔드포인트의 일부가 됩니다 (`https://RESOURCE_NAME.openai.azure.com/`).

2. [Azure AI Foundry](https://ai.azure.com/)로 이동하여 모델을 배포하세요.

   :::note
   opencode가 올바르게 작동하려면 배포 이름이 모델 이름과 일치해야 합니다.
   :::

3. `/connect` 명령을 실행하고 **Azure**.
...
4. 인증 방법을 선택하세요.
   ...
   - **API 키**: 리소스의 `KEY 1` 또는 `KEY 2`를 붙여넣으세요.
   - **Microsoft Entra ID (az login을 통한 OAuth)**: 먼저 `az login`을 실행하세요. 로그인한 Azure ID에는 해당 리소스에 대한 `Cognitive Services OpenAI User` 역할이 있어야 합니다.
...
6. 선택 사항: `/connect` 중 리소스 이름 프롬프트를 건너뛰려면 리소스 이름을 환경 변수로 설정하세요.
...
7. `/models` 명령을 실행하여 배포된 모델을 선택하세요.
...
1. [Azure 포털](https://portal.azure.com/)로 이동하여 **Azure OpenAI** 리소스를 만드세요. **리소스 이름**이 필요하며, 이는 API 엔드포인트의 일부가 됩니다 (`https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`).
...
3. `/connect` 명령을 실행하고 **Azure Cognitive Services**.
