...
1. Head over to the [Azure portal](https://portal.azure.com/) and create an **Azure OpenAI** resource. You'll need the **Resource name**; this becomes part of your API endpoint (`https://RESOURCE_NAME.openai.azure.com/`). When using Microsoft Entra ID, also copy the full **Resource ID** from the resource's **JSON View**. It contains the subscription, resource group, and resource name that OpenCode needs to list deployments.

   To find the active subscription and tenant:

   ```bash
   az account show \
     --query '{subscriptionName:name, subscriptionId:id, tenantId:tenantId}' \
     --output table
   ```

   If the wrong subscription is active, list the available subscriptions and select one:

   ```bash
   az account list \
     --query '[].{name:name, subscriptionId:id, active:isDefault}' \
     --output table

   az account set --subscription SUBSCRIPTION_ID
   ```

   Then list the Azure AI resources in that subscription. The output contains the resource name, resource group, and complete Resource ID; you do not need to construct the ID manually.

   ```bash
   az cognitiveservices account list \
     --subscription SUBSCRIPTION_ID \
     --query '[].{name:name, resourceGroup:resourceGroup, resourceId:id}' \
     --output json
   ```

   You can verify the deployments before connecting OpenCode:
...
4. Choose an auth method.
   ...
   - **Microsoft Entra ID (Azure CLI)**: [Install the Azure CLI](https://learn.microsoft.com/en-us/cli/azure/install-azure-cli), run `az login`, then assign the signed-in identity the `Cognitive Services OpenAI User` role on the Azure OpenAI resource under **Access control (IAM)**. `Owner` or `Contributor` alone does not grant inference access. Role assignments may take a few minutes to propagate.
...
5. Enter the full Resource ID when using Microsoft Entra ID. This lets OpenCode filter `/models` to successful deployments on that resource. A plain resource name remains supported, but cannot be used for deployment discovery. If you chose API key, enter the resource name and API key when prompted.

   ```txt
   ┌ Enter Azure Resource ID or Resource Name
   │
   │ /subscriptions/SUBSCRIPTION_ID/resourceGroups/RESOURCE_GROUP/providers/Microsoft.CognitiveServices/accounts/RESOURCE_NAME
   └ enter
   ```

6. Optional: set the Resource ID as an environment variable to skip the resource prompt during `/connect`.
   ...
   ```bash title="~/.bash_profile"
   export AZURE_RESOURCE_ID=/subscriptions/SUBSCRIPTION_ID/resourceGroups/RESOURCE_GROUP/providers/Microsoft.CognitiveServices/accounts/RESOURCE_NAME
   ```

   `AZURE_RESOURCE_NAME` remains supported when deployment filtering is not required.

7. Run the `/models` command to select your deployed model. With Entra ID and a Resource ID, OpenCode retrieves the successful deployments through Azure Resource Manager and hides undeployed catalog entries.
...
1. In [Microsoft Foundry](https://ai.azure.com/), create a **Foundry resource** and a project. Copy the **Project endpoint** from the project's overview page. It looks like `https://RESOURCE_NAME.services.ai.azure.com/api/projects/PROJECT_NAME`.

   If you know the subscription, resource group, and resource name, the Azure CLI can list the projects and their endpoints:
   ...
   az cognitiveservices account project list \
     ...
     --resource-group RESOURCE_GROUP \
     --name RESOURCE_NAME \
     --query '[].{project:name, endpoint:properties.endpoints."AI Foundry API"}' \
     --output table
   ```

   Use the value in the `Endpoint` column. The subscription and resource discovery commands in the Azure OpenAI section above can be used to find the other values.

2. Deploy a model from the Foundry model catalog.

   :::note
   The deployment name must exactly match the OpenCode model ID, for example `gpt-5-mini` or `claude-haiku-4-5`.
   :::

   :::note
   Partner models such as Claude require Azure Marketplace consent and may have no default quota for your subscription. If Foundry shows `0` TPM, request quota before creating the deployment.
   :::
...
4. Choose an auth method.
   - **API key**: Paste either `KEY 1` or `KEY 2` from your resource.
   - **Microsoft Entra ID (Azure CLI)**: [Install the Azure CLI](https://learn.microsoft.com/en-us/cli/azure/install-azure-cli), run `az login`, then assign the signed-in identity the `Foundry User` role on the resource or project under **Access control (IAM)**. `Owner` or `Contributor` alone does not grant inference access. Role assignments may take a few minutes to propagate.
