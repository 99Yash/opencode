...
1. Head over to the [Azure portal](https://portal.azure.com/) and create an **Azure OpenAI** resource. You'll need the **Resource name**; this becomes part of your API endpoint (`https://RESOURCE_NAME.openai.azure.com/`). With Microsoft Entra ID, OpenCode uses the active Azure CLI subscription to find the resource and its deployments automatically. A full **Resource ID** is only needed as an explicit override.
   ...
   You can list the Azure AI resources in the active subscription to verify the resource name. The output also contains the resource group and complete Resource ID; you do not need to construct the ID manually.
...
5. Enter the Resource name. With Microsoft Entra ID, OpenCode resolves it to a full Resource ID through the active Azure CLI subscription and filters `/models` to successful deployments. You can still enter a full Resource ID to bypass discovery. If you chose API key, enter the resource name and API key when prompted.
   ...
   If OpenCode cannot find the resource, use `az account set --subscription NAME_OR_ID` to select the subscription that owns it and reconnect.
...
6. Optional: set the Resource name as an environment variable to skip the resource prompt during `/connect`.
   ...
   For CI, cross-subscription setups, or an explicit override, set the complete Resource ID instead:
...
7. Run the `/models` command to select your deployed model. With Entra ID, OpenCode retrieves the successful deployments through Azure Resource Manager and hides undeployed catalog entries.
