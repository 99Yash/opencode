...
:::note
Wenn Fehler wie "I'm sorry, but I cannot assist with that request" auftreten, ändere in deiner Azure-Ressource den Inhaltsfilter von **DefaultV2** auf **Default**.
:::

1. Gehe zum [Azure-Portal](https://portal.azure.com/) und erstelle eine **Azure OpenAI**-Ressource. Du benötigst den **Ressourcennamen**; er wird Teil deines API-Endpunkts (`https://RESOURCE_NAME.openai.azure.com/`).

2. Gehe zu [Azure AI Foundry](https://ai.azure.com/) und stelle ein Modell bereit.

   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit opencode korrekt funktioniert.
   :::

3. Führe den Befehl `/connect` aus und suche nach **Azure**.

   ```txt
   /connect
   ```

4. Wähle eine Authentifizierungsmethode.

   - **API-Schlüssel**: Füge `KEY 1` oder `KEY 2` aus deiner Ressource ein.
   - **Microsoft Entra ID (OAuth über az login)**: Führe zuerst `az login` aus. Die angemeldete Azure-Identität muss für die Ressource die Rolle `Cognitive Services OpenAI User` haben.

   ```txt
   ┌ Select auth method
   │
   │ API key
   │ Microsoft Entra ID (OAuth via az login)
   └
   ```

5. Wenn du API-Schlüssel gewählt hast, gib deinen API-Schlüssel ein.
...
6. Optional: Lege den Ressourcennamen als Umgebungsvariable fest, um die Abfrage des Ressourcennamens während `/connect` zu überspringen.

   ```bash
   AZURE_RESOURCE_NAME=XXX opencode
   ```

   Oder füge ihn deinem Bash-Profil hinzu:
...
1. Gehe zum [Azure-Portal](https://portal.azure.com/) und erstelle eine **Azure OpenAI**-Ressource. Du benötigst den **Ressourcennamen**; er wird Teil deines API-Endpunkts (`https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`).
...
3. Führe den Befehl `/connect` aus und suche nach **Azure Cognitive Services**.
