...
:::note
إذا واجهت أخطاء مثل "أنا آسف، لكن لا يمكنني المساعدة في هذا الطلب"، فجرّب تغيير عامل تصفية المحتوى من **DefaultV2** إلى **Default** في مورد Azure الخاص بك.
:::

1. انتقل إلى [مدخل Azure](https://portal.azure.com/) وأنشئ مورد **Azure OpenAI**. ستحتاج إلى **اسم المورد**؛ ويصبح هذا جزءًا من نقطة نهاية API الخاصة بك (`https://RESOURCE_NAME.openai.azure.com/`).

2. انتقل إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجًا.

   :::note
   يجب أن يتطابق اسم النشر مع اسم النموذج حتى يعمل opencode بشكل صحيح.
   :::
...
4. اختر طريقة المصادقة.
   ...
   - **مفتاح API**: الصق `KEY 1` أو `KEY 2` من موردك.
   - **Microsoft Entra ID (OAuth عبر az login)**: شغّل `az login` أولًا. يجب أن تكون هوية Azure التي سجّلت الدخول بها مالكة لدور `Cognitive Services OpenAI User` لهذا المورد.
...
5. إذا اخترت مفتاح API، فأدخل مفتاح API الخاص بك.
...
6. اختياري: عيّن اسم المورد كمتغير بيئة لتخطي طلب اسم المورد أثناء `/connect`.

   ```bash
   AZURE_RESOURCE_NAME=XXX opencode
   ```

   أو أضفه إلى ملف تعريف bash الخاص بك:

   ```bash title="~/.bash_profile"
   export AZURE_RESOURCE_NAME=XXX
   ```

7. شغّل الأمر `/models` لتحديد النموذج المنشور.
...
1. انتقل إلى [مدخل Azure](https://portal.azure.com/) وأنشئ مورد **Azure OpenAI**. ستحتاج إلى **اسم المورد**؛ ويصبح هذا جزءًا من نقطة نهاية API الخاصة بك (`https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`).
