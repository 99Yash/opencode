...
`prompt` durably admits input before scheduling execution; `resume: false` admits only. `resume()` starts or joins execution, `interrupt()` requests user interruption, and `wait()` waits for local execution to settle without starting it. Child Sessions created by subagent tools inherit their parent's instance. Closing the owning Scope settles bound execution before disposing capabilities, preserves shutdown recovery claims, and makes retained handle methods fail with `Session.ClosedError`. Keep that Scope open for the lifetime of an active thread, not just one inbound message.
