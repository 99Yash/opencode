...
it completes. Commands targeting primary or all-mode agents continue to run in
the current session. The legacy `subtask` field remains accepted but does not
override the selected agent's mode.
