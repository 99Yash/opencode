...
<Callout type="note">
  OpenCode 1 and OpenCode 2 can be installed side by side. V1 runs as `opencode`, while V2 installs and runs separately
  as `opencode2`.
</Callout>
...
`template`, `description`, and `agent` keep their names. Rename `subtask` to `subagent`; V2 still accepts `subtask` as an alias.
Existing Markdown command definitions remain supported.
...
the command template, and `description` and `agent` frontmatter keep the same names. Rename `subtask` to `subagent`; the old
name remains accepted as an alias. If frontmatter has separate
