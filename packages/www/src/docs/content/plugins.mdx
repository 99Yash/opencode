...
Git plugins once. Use the CLI's [Plugins dialog](/cli/plugins#manage-plugins) to explicitly check, update, or reload a package
again without restarting the service or reopening the terminal. Exact npm versions and full Git commit hashes stay pinned.
Changes to unwatched local dependencies may still require restarting OpenCode.
