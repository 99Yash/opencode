...
```ts
setup: async (ctx) => {
  const task = synchronize(ctx, ctx.signal)
...
connections, and background tasks. `ctx.signal` is aborted before cleanup runs.
Runtime hooks receive an invocation signal as their second callback argument,
and tool executors receive it on their context.
...
`Plugin.define` compiles this authoring API to the same Promise contract used by
other plugins. Effects, callbacks, scopes, and schema conversion stay inside the
plugin's installed Effect runtime; OpenCode receives Promise capabilities and
detached Standard Schema implementations. This means the plugin and OpenCode do
not need to share one physical `effect` package instance.
...
return an Effect and may fail with the typed tool failure channel. Input schemas
decode model JSON before execution; output schemas encode executor values after
execution.
