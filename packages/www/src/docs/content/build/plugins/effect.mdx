...
Call `invalidate` when external state used by a transform changes. It marks the domain dirty and returns. The next
domain read replays every transform in order before returning; if no read arrives first, OpenCode rebuilds in the
background. Use `reload` when the producer itself must wait for that debounced rebuild.
