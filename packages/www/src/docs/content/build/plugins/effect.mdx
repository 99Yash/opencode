...
`list()` returns `[id, tool]` pairs. `get`, `update`, and `remove` use the effective registry name, such as
`acme_greeting` above. `get` returns `undefined` for missing tools; `update` and `remove` do nothing. Creating a tool
requires `add`, not an agent-style upsert. Updates preserve the name and namespace; assign new schemas and options to
replace them. Updates and removals are scoped overrides, just like added tools, and apply once when registered.
