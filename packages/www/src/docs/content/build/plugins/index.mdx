...
Register, inspect, update, and remove tools with a transform.
...
`list()` returns `[id, tool]` pairs and `get(id)` returns an executable definition or `undefined`. IDs are effective
registry names, including the namespace: `acme_greeting` in the example above. Dots in namespaces and unsupported
characters in tool names become `_`.
...
Updates and removals are scoped overrides, just like added tools. They apply once when registered, not on a replay.
