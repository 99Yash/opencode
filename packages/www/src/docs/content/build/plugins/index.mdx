...
Now say the first plugin fetches its model list from a dynamic source. It can
call `invalidate` when that list changes.
...
`invalidate` marks the catalog dirty and returns. The next catalog read replays every transform in order before
returning, so the output-price policy still filters the refreshed models without making the producer wait for the
debounce window. If no read arrives first, OpenCode rebuilds the catalog in the background. Use `reload` when the
producer itself must wait for that debounced rebuild.
