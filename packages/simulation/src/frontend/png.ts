...
export function screenshotFrame(frame: CapturedFrame) {
    ...
    line.spans.forEach((span) => {
      ...
      chars.forEach((char, index) => {
        ...
        if (!hidden && char.codePointAt(0) !== 0x0a00) {
          ...
          if (attributes & TextAttributes.UNDERLINE) context.fillRect(x, y + 17, cells * CellWidth, 1)
          if (attributes & TextAttributes.STRIKETHROUGH) context.fillRect(x, y + 10, cells * CellWidth, 1)
        }
        column += cells
        remaining -= cells
      })
  ...
  return { width: canvas.width, height: canvas.height, data: canvas.toBuffer("image/png") }
}
