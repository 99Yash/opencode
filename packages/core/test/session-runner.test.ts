...
const layer = Layer.unwrap(
  Effect.map(RunnerState, (state) => {
    ...
    const pluginSupervisor = Layer.succeed(PluginSupervisor.Service, {
      ...noopPluginSupervisor(Effect.suspend(() => state.pluginFlushHook)),
      flush: Effect.suspend(() => state.pluginConfigFlushHook),
    })
...
describe("SessionRunnerLLM", () => {
  ...
  scenario("defers plugin activation through retry backoff and blocks a new Session drain", function* (s) {
    const gate = yield* PluginExecution.Service
    yield* s.admit("Retry before updating plugins")
    yield* insertSession(otherSessionID)
    yield* s.session.prompt({ sessionID: otherSessionID, text: "Wait for replacement", resume: false })
    yield* s.llm.push(
      Stream.fail(providerUnavailable()),
      TestLLM.text("Recovered", "text-retry"),
      TestLLM.text("New generation", "text-new"),
    )
    const scheduled = yield* s.bus
      .subscribe(SessionEvent.RetryScheduled)
      .pipe(Stream.runHead, Effect.forkScoped({ startImmediately: true }))
    const running = yield* s.resume.pipe(Effect.forkScoped({ startImmediately: true }))
    yield* Fiber.join(scheduled)
    const reconciled = yield* Deferred.make<void>()
    s.pluginConfigFlushHook = Deferred.await(reconciled)
    let activated = false
    ...
    const waiting = yield* s.session.resume(otherSessionID).pipe(Effect.forkScoped({ startImmediately: true }))
    yield* Effect.yieldNow
